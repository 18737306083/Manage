var bookappointment={
		...
		URL:{
			appoint:function(bookId,studentId){
				return '/BookService/books/'+bookId+'/appoint?studentId='+studentId;
			},
			verify:function(){
				return '/BookService/books'+'/verify';
			}
		},
		...
		appointment:function(bookId,studentId, node){
			console.log("我执行借阅的方法!" );
			node.html('<button class="btn btn-primary btn-lg" id="appointmentBtn">借阅</button>');
			...
			$('#appointmentBtn').one('click', function () {
				...
				$.post(appointmentUrl,{},function(result){   //Ajax强大之处，向Controller方法提出请求和返回结果在一处!
					if(result && result['success']){         //同时还可以连续取对象的子对象！
						...
						//显示借阅结果                                                          把结果显示给页面，完成了jsp的工作
							 
						node.html('<span class="label label-success">'+stateInfo+'</span>');
					}       //因为公用一个node所以，用来显示“stateInfo”时就不会显示上面的“借阅”
